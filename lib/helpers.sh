...
curl_get() {
  ...
  if [[ "$DRY_RUN" == "true" ]]; then
    log "[dry-run] curl_get called for URL: $url"
    return 0
  fi

  while (( attempt <= max_attempts )); do
    if ! resp=$(curl "${curl_opts[@]}" "$url" 2>"$CURL_ERR_LOG"); then
      err_msg=$(<"$CURL_ERR_LOG")
...
curl_head_code() {
  ...
  if [[ "$DRY_RUN" == "true" ]]; then
    >&2 echo "[dry-run] curl_head_code called for URL: $url"
    echo "000"
    return 0
  fi

  while (( attempt <= max_attempts )); do
    if ! code=$(curl "${curl_opts[@]}" -w "%{http_code}" -o /dev/null "$url" 2>"$CURL_ERR_LOG"); then
      err_msg=$(<"$CURL_ERR_LOG")
...
record_finding() {
  ...
  jq -n --arg ts "$ts" --arg domain "$domain" --arg url "$url" \
        ...
        '{timestamp:$ts,domain:$domain,url:$url,test:$test,detail:$detail,extra:$extra}' \
        >> "$JSON_TMP"
